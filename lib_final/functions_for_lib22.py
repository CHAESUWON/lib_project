...
def return_result_from_Oracle():
    ...
    cursor = conn.cursor()
    sql = """select ID, 등록번호, 도서명, 저자, 소장도서관명, 출판사, 출판연도, 청구기호, 대여시작일 from  test_table ORDER BY ID """ 
...
def search_book(book_to_rent):
    while True:
        print("1. 도서명 검색 -")
        print("2. 저자명 검색 -")
        print("3. 도서 위치 찾기 - ") # 본인 도서관 어디에 소장되어 있는지, 타 도서관에 있는지
        print("Q. 뒤로가기")
        ...
        if num_input == '1': 
            ...
            if abc != []:
                book_number_input = input("\n장바구니에 담을 책의 번호를 입력하세요. (나가려면 Q 입력) >> ")
                print("\n")
...
def return_book(log_in_ID):
    while True:
        ...
        # 1. 책 반납
        if user_input == '1':
            print("-"*10, f"[{log_in_ID}] 의 빌린 책 목록", "-"*16)
            count = 1
            for row in return_result_from_Oracle():
                # 아이디 입력받아와야함
                if row[0] == log_in_ID:
